...
export const ChatMessage = ({ message, onSourceClick, onFollowUpClick, botAvatar }: ChatMessageProps) => {
  ...
  return (
    <div
      className={cn(
        "flex w-full mb-4 sm:mb-6 animate-pop-in",
    ...
    >
      <div className={cn(
        ...
        <div className="relative shrink-0">
          <Avatar className={cn(
            "w-7 h-7 sm:w-8 sm:h-8 shadow-sm bg-gradient-to-br relative",
            isUser
            ...
            <AvatarFallback className="bg-transparent">
              {isUser ? (
                ...
                <BotAvatarIcon avatar={botAvatar} showEffects={false} className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
              )}
            </AvatarFallback>
          </Avatar>
        </div>
        ...
        <div className={cn("min-w-0 font-sans", isUser ? "" : "flex-1")}>
          <div className={cn(
            "relative px-3 py-2.5 sm:px-5 sm:py-3.5 shadow-sm transition-all duration-200",
            isUser
              ? "bg-gradient-to-br from-primary to-emerald-600 text-white rounded-2xl rounded-tr-sm mx-1 sm:mx-2 animate-pop-in"
          ...
          {!isUser && !isStreaming && message.sources && message.sources.length > 0 && (
            <div className="mt-2 sm:mt-3 ml-0 sm:ml-1 animate-fade-in group">
              ...
              <div className="space-y-1.5 sm:space-y-2 relative">
                <div className="absolute -left-3 top-0 bottom-0 w-0.5 bg-gradient-to-b from-border/50 to-transparent rounded-full hidden sm:block" />
